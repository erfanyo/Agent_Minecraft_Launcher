...
class PluginCreationGuide(Skill):
    ...
    def ai_hint(self) -> str:
        return ("【插件生成指南】启动器支持插件 = plugins/<名字>.py,提供 register(api),可注册 "
                ...
                "② 生成:**必须调用 create_plugin(name, code) 工具把源码落盘到 plugins/**,"
                "**不要只输出 Python 源码文本给用户**(那样不能生效;create_plugin 会校验语法+写文件)。\n"
                "   code 参数 = 完整插件源码(必须含 register(api) 函数);这一轮工具调用没成功就再试一次。\n"
                "③ 写完后告诉用户【重启启动器生效】(静态加载;生成后不立即生效)。\n"
