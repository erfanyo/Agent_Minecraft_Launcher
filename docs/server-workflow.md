...
- 服务端管理菜单的“启动测试并逐项修复”先走正常启动流程，包括由用户本人确认 EULA。检测到本次会话的 Minecraft 就绪日志后结束测试；仅凭进程存在不算通过。
- 启动失败时只分析本次托管日志。若日志及 JAR 内部元数据共同指向某个仅客户端 Mod，逐项显示文件、证据和停用的功能代价，由用户逐次确认；停用是 `.jar.disabled` 重命名，原 JAR 保留，随后自动重启验证。
- Forge Mixin 崩溃没有直接点名 Mod 时，只有日志注入标记与唯一 JAR 的 Mixin 配置吻合才会提出候选；这类证据仍标为“很可能”。
- 若无明确客户端适用端证据、用户拒绝、启动正常退出却没有就绪标记，循环停止。依赖缺失、版本冲突等交给 AI 辅助诊断，AI 不负责自动决定停用或绕过确认。
- 同一启动器会话中，可在服务端管理菜单点击“恢复本次测试停用的 Mod”逐个重新启用；服务端运行期间不能恢复。
- 测试状态只在当前启动器会话保留；关闭启动器会结束自动循环，已启动服务端仍由独立托管进程管理。从“自动转换为候选服务端”进入测试时，结束后会把用户确认排除的 Mod、验证状态及清单同步回导出 ZIP；仅从服务端菜单单独测试不会修改原 ZIP。
- 导出完成会询问是否测试；选“否”直接保留未验证的候选 ZIP。选“是”会导入隔离实例、经用户确认 EULA 和每次停用，结束后询问是否移除新建的测试实例；移除会先安全停服，再移入 `servers/.trash`，原 ZIP 不受影响。复用已有服务端时不提供删除选项。
