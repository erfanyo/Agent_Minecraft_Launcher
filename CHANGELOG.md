...
- **📖 新手教程(基础版,模块化)**:灵感 #7 第①部分启动器教程落地。**内容数据**(`tutorial_content.py`)+**通用渲染器**(`tutorial_gui.py`),与 UI 解耦——将来大改 UI 只改内容数据,不用重构渲染器。按页面讲"每个控件在哪、有什么用":**基本概念**(实例 vs 版本 / 离线账号 / 资源类型速览)、我的版本首页、下载新资源、实例管理、启动器设置、AI 助手面板、常见问题(共 6 页 + 3 概念、48 条说明)。入口:菜单「帮助 → 📖 新手教程…」+「我的版本」首页「📖 新手教程」按钮。
