...
> AI 回答更贴合当前场景(仅注入上下文,不授予操作权限)。
> 其余:后台任务/启动服务重构、实例重命名、已安装资源卡片、CurseForge、崩溃诊断/日志脱敏、
> bridge-mod Forge 1.12.2。用户向说明见 `RELEASE_NOTES_0.7.0.md`。
...
- Windows 上游戏目录或启动器目录含中文时，仅将 AMCL 管理的 Java 自动放到安全的用户运行时目录；实例名称和游戏文件仍可保留中文，并避免把路径问题误判为支持库缺失。
...
### 📄 许可
- 项目从 GNU AGPL-3.0 调整为 **GNU GPL-3.0-only**。既有版本已获得的 AGPL-3.0 授权不受影响。

## [v0.6.0] - 2026-09-01

> 玩家版更新说明见 `RELEASE_NOTES_0.6.0.md`；本节为专业版变更记录。

### 修复

- `loaders.py`：Forge/NeoForge processor 期间为 `_versions` 中的继承根临时建立
  `versions/<mc>/<mc>.jar`，完成或失败后仅清理本次创建的副本；修复 Forge 1.18.2 第 5 步硬编码路径导致的补丁失败。
- `main.py`：关闭版本隔离时 `_delete_instance()` 过去错误删除 `game_dir_for()` 返回的整个 `.minecraft`；删除目标固定为 `versions/<id>`。
- `main.py`：基础原版迁移不再依据共用 gameDir 的存档目录，而只迁移仍为 `json + jar` 最小形态的未启动版本目录。

### 功能与兼容

- Forge 1.18.2：独立 Gradle 7 / JDK 17 构建通过，提供本地指令、结果回传、`/ai`、权限上下文、物品与配方导出。
- 启动前可同步 Minecraft `options.txt` 的语言到启动器/系统语言（默认开启）。
- 开始 Forge 1.16.5 的 Gradle 6.8.3 / JDK 8 独立工具链；已完成官方映射初始化，待旧包名 API 专用适配。

### Bridge Mod 兼容矩阵

| 加载器 | MC 版本 | 状态 | 能力 |
|---|---:|---|---|
| Fabric | 1.21.1 | 已有产物，待复核 | 完整桥接目标 |
| Forge | 1.20.1 | 已构建验证 | 指令、`/ai`、权限、物品/配方导出 |
| Forge | 1.19.2 | 已构建验证 | 指令、`/ai`、权限、物品/配方导出 |
| Forge | 1.18.2 | 已构建验证 | 指令、`/ai`、权限、物品/配方导出 |
| Forge | 1.16.5 | 工具链完成，适配中 | 目标为完整桥接 |
| Forge | 1.12.2 / 1.7.10 | 规划中 | 旧版兼容实现 |

### 功能

- **Minecraft 语言跟随启动器/系统**：新增默认开启的同步开关。启动实例前只更新该实例
  `options.txt` 的 `lang` 项；关闭后完全保留玩家在游戏内设定的独立语言。
- **bridge-mod Forge 1.19.2**：独立 ForgeGradle 5 / Gradle 7.6.4 构建链已验证，保留本地
  指令口、`/ai`、权限上报、配方与物品导出；与现代 Gradle 8 工程互不干扰。

### 文档

- **bridge-mod 兼容策略修正**：旧版（1.16.5 / 1.12.2 / 1.7.10）不是“不计划”或主动阉割，
  而是因旧 Forge/Minecraft API 需要独立 JDK 8 与 ForgeGradle 工程的兼容实现；目标是在各版本
  能力范围内尽量提供完整 API。短期目标版本统一为 1.21.1、1.20.1、1.19.2、1.18.2、1.16.5、1.12.2、1.7.10。
- **加载器优先级明确**：对 1.21.1 以下版本统一按 **Forge > Fabric > NeoForge** 排期；这不影响
  启动器下载/安装对其他加载器和版本的兼容，只决定 bridge-mod 的开发与发布先后。
